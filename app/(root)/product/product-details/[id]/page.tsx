            ...
            <div className="w-4/5 mx-auto grid grid-cols-1 lg:grid-cols-7 items-center gap-4">
                ...
                <div className="col-span-4">
                    ...
                    <div className="mt-2 flex items-center space-x-2">
                        {starArray.map((star) => {
                            return (
                                <StarIcon key={Math.random() * 5000} size={20} fill="yellow" className="text-yellow-600" />
                            );
                        })}
                    </div>
                    ...
                    <h1 className="lg:text-6xl text-3xl text-blue-950 font-bold">${singleProduct?.price.toFixed(2)}</h1>
                    ...
                    <p className="mt-2 text-sm text-black text-opacity-70 font-semibold">Tage: Shop,WDW</p>
                    <p className="mt-2 text-sm text-black text-opacity-70 font-semibold">SKU : {Math.random() * 500}</p>
                    <CartButton product={singleProduct}/>
                </div>
            </div>
