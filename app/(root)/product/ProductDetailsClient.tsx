...
const ProductDetailsClient = ({ product, relatedProduct }: { product: Product; relatedProduct: Product[] }) => {
    ...
    return (
        <div className="w-4/5 mx-auto grid grid-cols-1 lg:grid-cols-7 items-center gap-4">
            <Button variant="link" onClick={() => router.back()} className="gap-1 cursor-pointer">
                <ChevronLeftIcon className="opacity-60" size={16} aria-hidden="true" />
                Go back
            </Button>
            <div className="col-span-3 mb-6 lg:mb-0">
                {product.additionalImages.map((imgUrl: string, index: number) => (
                    <div>
                        <Image key={index} src={imgUrl} alt={`${product.title} ${index + 1}`} width={400} height={400} />
                    </div>
                ))}
            </div>
